...
final class ExperimentsCollectionViewController: CollectionViewController, ExperimentController, DeviceIsChosenDelegate, UIPopoverPresentationControllerDelegate {
    ...
    private let emptyView = EmptySplashView()
    ...
    override func viewDidLoad() {
        ...
        emptyView.bringSubviewToFront(view)
        emptyView.anchor(top: view.topAnchor, left: view.leftAnchor, bottom: view.bottomAnchor, right: view.rightAnchor, paddingTop: 150, paddingLeft: 30, paddingBottom: 100, paddingRight: 30, width: 0, height: 0, enableInsets: false)
        emptyView.onWebLinkCallback = {
            let url = localize("emptySpalshWebLink")
    ...
    @objc func reload() {
        ...
        for temp in collections {
            emptyView.isHidden = !temp.experiments.isEmpty
            if !temp.experiments.isEmpty {
                break
            }
        }
    }
